...
### 3.3 AMD GPU Configuration (MI300X / MI325X / MI355X)

**FP8 deployment (8×MI300X, TP=8/EP=8):**

```shell Command
SGLANG_USE_AITER=1 \
SAFETENSORS_FAST_GPU=1 \
sglang serve Qwen/Qwen3-VL-235B-A22B-Instruct-FP8 \
  --tp 8 \
  --ep 8 \
  --mem-fraction-static 0.94 \
  --context-length 32768 \
  --max-running-requests 10240 \
  --chunked-prefill-size 16384 \
  --max-prefill-tokens 32768 \
  --kv-cache-dtype fp8_e4m3
```

> The above configuration runs efficiently at high concurrencies (up to 512) for context lengths up to 8k. If TPOT is not satisfactory, lower `--max-prefill-tokens`. For longer contexts, `--max-running-requests` and `--max-prefill-tokens` may need retuning. For throughput-first deployments where TTFT latency is not a constraint, TP=4/EP=4 can yield higher output throughput for short-output workloads.
